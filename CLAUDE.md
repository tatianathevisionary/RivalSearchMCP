...
### How it works
- `SKILL.md` tells the agent what tools exist and how to invoke them via `cli.py`
- `cli.py` was generated by `fastmcp generate-cli` and connects to the live server at `https://RivalSearchMCP.fastmcp.app/mcp`
- Resource files in `resources/` provide detailed flag reference (only loaded when the agent needs specifics)
- The CLI is self-contained with PEP 723 inline dependencies — `uv run cli.py` just works
