    ...
    async def search(
        ...
        try:
            ...
            if tag:
                ...
                feed_url = f"{self.base_url}/search?q={quote_plus(query)}"
            ...
            async with httpx.AsyncClient(headers=headers, timeout=30.0) as client:
                # Try RSS feed first
                try:
                    rss_url = f"{self.base_url}/feed/tag/{query.lower().replace(' ', '-')}"
                ...
                if not articles:
                    ...
                    article_links = soup.find_all('a', href=lambda h: h and ('/p/' in h or '/@' in h))
