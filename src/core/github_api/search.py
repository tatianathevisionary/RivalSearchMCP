...
class GitHubSearch:
    ...
    def _check_rate_limit(self):
        ...
        if self.request_count >= 55:  # Leave buffer of 5
            logger.warning(f"GitHub API rate limit approaching: {self.request_count}/60 requests used")
            if self.request_count >= 60:
                remaining_time = 3600 - (current_time - self.last_reset_time)
                logger.error(f"GitHub API rate limit exceeded. Resets in {remaining_time/60:.1f} minutes")
                raise Exception(f"GitHub API rate limit exceeded. Try again in {remaining_time/60:.1f} minutes")
