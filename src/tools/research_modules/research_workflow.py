...
async def execute_research_tool(tool_name: str, arguments: Dict[str, Any]) -> str:
    ...
    try:
        ...
        if tool_name == "web_search":
            ...
            results_summary = (
                f"Found {result.get('summary', {}).get('total_results', 0)} search results:\n"
            )
                ...
                "parameters": {
                    ...
                    "properties": {
                        ...
                        "platforms": {
                            ...
                            "items": {
                                "type": "string",
                                "enum": ["reddit", "hackernews", "devto", "producthunt", "medium"],
                            },
                            "description": "Platforms to search",
                            "default": ["reddit", "hackernews"],
                        },
                        "max_results_per_platform": {"type": "integer", "default": 5},
                    },
                    "required": ["query"],
                },
            },
...
class ResearchWorkflowExecutor:
    ...
    async def execute_research_workflow(
        ...
        # Get API key - required for AI agent (no hardcoded fallback)
        api_key = os.getenv("OPENROUTER_API_KEY", "").strip()
        if not api_key and enable_ai_insights:
            if ctx:
                await ctx.info("⚠️ OPENROUTER_API_KEY not set - falling back to basic research")
        ...
        try:
            if enable_ai_insights:
                ...
                if ctx:
                    await ctx.info(
                        f"🤖 AI Agent initialized with {len(free_models)} models (top 3 of {len(all_free_models)})"
                    )
...
Begin your research now.""",
                ...
                if ctx:
                    await ctx.info(
                        f"🎯 Research completed! Used {len(research_results['tool_calls'])} tool calls"
                    )
            ...
            error_msg = str(e)
            if "User not found" in error_msg or "401" in error_msg or "403" in error_msg:
                error_msg = (
                    "OpenRouter API authentication failed. Set a valid OPENROUTER_API_KEY "
                    "at https://openrouter.ai/keys and try again."
                )
            else:
                error_msg = f"Research workflow failed for '{topic}': {error_msg}"
            if ctx:
    ...
    async def _execute_legacy_research(
        self, topic: str, max_sources: int, research_id: str, ctx: Context = None
    ) -> str:
        """Legacy research workflow for backward compatibility when AI is disabled."""
        if ctx:
            await ctx.info("📝 Running basic research (AI disabled)")

        # Use multi-engine search for better coverage (DuckDuckGo + Yahoo + Wikipedia)
        from src.tools.multi_search import get_orchestrator

        orchestrator = get_orchestrator()
        search_response = await orchestrator.search_all_engines(
            query=topic,
            num_results=max(5, max_sources),
            extract_content=False,
            follow_links=False,
            max_depth=1,
        )

        results_data = search_response.get("results", {})
        deduplicated = results_data.get("deduplicated", {}).get("results", [])
        sources = deduplicated[:max_sources] if isinstance(deduplicated, list) else []
        ...
        if sources:
            for i, result in enumerate(sources, 1):
                item = result if isinstance(result, dict) else result.to_dict()
            ...
            engines_tried = summary.get("engines_searched", 0)
            output += (
                f"No sources found from {engines_tried} search engines. "
                "Try refining your topic or using a different query.\n\n"
            )
    ...
    async def _execute_website_analysis(self, research_results: Dict[str, Any], topic: str) -> None:
        ...
        try:
            ...
            website_analysis = []
            sources_to_analyze = (
                research_results["phases"].get("source_discovery", {}).get("sources", [])[:5]
            )
    ...
    async def _generate_synthesis_and_insights(
        ...
        try:
            ...
            research_results["metadata"]["successful_phases"] = sum(
                1
                for phase in research_results["phases"].values()
            ...
            if ai_client and selected_model:
                await self._generate_ai_insights(research_results, ai_client, selected_model, ctx)
