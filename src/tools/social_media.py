...
def register_social_media_tools(mcp: FastMCP):
    ...
    async def social_search(
        query: str,
        platforms: List[Literal["reddit", "hackernews", "devto", "producthunt", "medium"]] = [
            "reddit",
            "hackernews",
            "devto",
        ],
        max_results_per_platform: int = 10,
        max_results: int = 0,  # Alias: when > 0 overrides max_results_per_platform (AI agent compat)
        ...
        try:
            ...
            if "reddit" in platforms:
                reddit_results = await reddit_search.search(
                    query=query, subreddit=reddit_subreddit, limit=limit, time_filter=time_filter
                )
                results["reddit"] = {
            ...
            if "hackernews" in platforms:
                hn_results = await hn_search.search(query=query, limit=limit)
                results["hackernews"] = {
            ...
            if "devto" in platforms:
                devto_results = await devto_search.search(query=query, per_page=limit)
                results["devto"] = {
            ...
            if "producthunt" in platforms:
                ph_results = await producthunt_search.search(query=query, limit=limit)
                results["producthunt"] = {
            ...
            if "medium" in platforms:
                medium_results = await medium_search.search(query=query, limit=limit)
                results["medium"] = {
